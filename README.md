...
默认只同时执行一个转换任务，其余任务进入队列。资源限制可以通过环境变量调整：

```text
MAIX_MAX_CONCURRENT_JOBS=1
MAIX_MAX_MODEL_MB=1024
MAIX_MAX_DATASET_MB=4096
MAIX_MAX_ZIP_ENTRIES=20000
MAIX_MAX_ZIP_UNCOMPRESSED_MB=12288
MAIX_MAX_ZIP_FILE_MB=1024
MAIX_MAX_ZIP_COMPRESSION_RATIO=200
MAIX_MAX_API_LOG_MB=64
MAIX_MAX_CONVERSION_LOG_MB=64
MAIX_MAX_LOG_RESPONSE_MB=8
```

Linux / macOS 可以在启动命令前设置这些变量；Windows CMD 使用 `set 变量名=值`，Windows PowerShell 使用 `$env:变量名 = "值"`。
